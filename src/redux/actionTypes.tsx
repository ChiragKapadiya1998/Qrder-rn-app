export const IS_LOADING = 'IS_LOADING';
export const IS_LOADING_NEW = 'IS_LOADING_NEW';
export const SET_APP_THEME = 'SET_APP_THEME';
export const USER_INFO = 'USER_INFO';
export const USER_LOGOUT = 'USER_LOGOUT';
export const GET_CITY_DATA = 'GET_CITY_DATA';
export const GET_CUISINES_DATA = 'GET_CUISINES_DATA';
export const GET_CHEFS_DATA = 'GET_CHEFS_DATA';
export const DELETE_MENU_DATA = 'DELETE_MENU_DATA';
export const GET_MENU_DATA = 'GET_MENU_DATA';
export const GET_EMPTY_MENU_LIST = 'GET_EMPTY_MENU_LIST';
export const GET_EMPTY_CANTEEN_LIST = 'GET_EMPTY_CANTEEN_LIST';
export const SEARCH_CITY = 'SEARCH_CITY';
export const SELECT_ROLE = 'SELECT_ROLE';
export const SET_APP_LANGUAGE = 'SET_APP_LANGUAGE';
export const SET_FOOD_VEG = 'SET_FOOD_VEG';
export const INCREMENT = 'INCREMENT';
export const DECREMENT = 'DECREMENT';
export const DELETE_CHEF_DATA = 'DELETE_CHEF_DATA';
export const DELETE_CUISINES_DATA = 'DELETE_CUISINES_DATA';
export const GET_MISCELLANEOUS = 'GET_MISCELLANEOUS';
export const GET_MENU_MASTERS = 'GET_MENU_MASTERS';
export const GET_RECIPES_MASTERS = 'GET_RECIPES_MASTERS';
export const GET_RECIPES_MENU = 'GET_RECIPES_MENU';
export const GET_DASHBOARD_LIST = 'GET_DASHBOARD_LIST';

export const GET_UNIVERSITIES_LIST = 'GET_UNIVERSITIES_LIST';
export const GET_UNIVERSITIES_CANTEEN_LIST = 'GET_UNIVERSITIES_CANTEEN_LIST';
export const GET_CANTEEN_MENU_LIST = 'GET_CANTEEN_MENU_LIST';
export const GET_CANTEEN_CUISINE_LIST = 'GET_CANTEEN_CUISINE_LIST';
export const GET_CARD_LIST = 'GET_CARD_LIST';
export const DELETE_CARD_LIST = 'DELETE_CARD_LIST';
export const GET_DISCOUNT = 'GET_DISCOUNT';
export const GET_SUPPORT_TYPE = 'GET_SUPPORT_TYPE';
export const GET_SEARCH = 'GET_SEARCH'

export const GET_RUNNING_ORDERS = 'GET_RUNNING_ORDERS';
export const GET_ORDER_REQUESTS = 'GET_ORDER_REQUESTS';
export const ACCPET_ORDER_REQUESTS = 'ACCPET_ORDER_REQUESTS';
export const ORDER_DECLINED = 'ORDER_DECLINED';
export const RUNNING_ORDER_COMPLETED = 'RUNNING_ORDER_COMPLETED';
export const GET_ALL_ORDER = 'GET_ALL_ORDER';
export const GET_ALL_STUDENT_ORDER = 'GET_ALL_STUDENT_ORDER';
export const GET_ALL_MY_ORDER = 'GET_ALL_MY_ORDER';
export const GET_CARD_EMPTY_LIST = 'GET_CARD_EMPTY_LIST';
export const GET_FIND_CITY_DATA = 'GET_FIND_CITY_DATA'
