...
import {
  ...
  deleteRecipeMasterAction,
  getCuisinesAction,
  getMenuMastersAction,
  getRecipeMasterAction,
  getRecipeMenusAction,
...
const RecipesMastersList = (props: Props) => {
  ...
  useEffect(() => {
    getCuisinesList(1);
    getMenuMastersList(1);
    getRecipeMenusList(1);
  ...
  const getCuisinesList = (pages: number) => {
    ...
    dispatch(getRecipeMasterAction(obj));
  };

  const getMenuMastersList = (pages: number) => {
    let obj = {
      data: {
        page: pages,
        limit: 100,
    ...
    dispatch(getMenuMastersAction(obj));
  };

  const getRecipeMenusList = (pages: number) => {
    let obj = {
      data: {
        page: pages,
        limit: 100,
    ...
    dispatch(getRecipeMenusAction(obj));
  };

  const onSearchBar = (text: string) => {
    setSearchQuery(text);
    const filteredItems = getRecipesMasters?.filter((f: any) =>
      f?.menu_name?.toLowerCase()?.match(text?.toLowerCase()),
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <HomeHeader
        ...
        onRightPress={() => {
          navigation.navigate(screenName.AddRecipesMasters);
          // setNewFolder(true)
        }}
        mainShow={true}
            ...
            renderItem={({item, index}) => {
              return (
                <CuisinesNameCardList
                  ...
                  item={item}
                  onPressEdit={() => {
                    navigation.navigate(screenName.EditRecipesMasters, {
                      data: item,
                    });
                    setSelectItem(item);
                  }}
