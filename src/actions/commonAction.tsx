...
export const searchMenuList =
  ...
  async dispatch => {
    ...
    return makeAPIRequest({
      method: GET,
      url: `${api.search_menu_list}/${request.data}`,
      headers: headers,
      ...
      .then(async (response: any) => {
        if (response.status === 200 || response.status === 201) {
          dispatch({type: IS_LOADING, payload: false});
          // dispatch({ type: GET_CANTEEN_MENU_LIST, payload: response?.data?.data?.data});
          dispatch({type: GET_SEARCH, payload: response?.data?.data?.data});
          if (request.onSuccess) request.onSuccess(response.data.data);
        }
      })
      .catch(error => {
